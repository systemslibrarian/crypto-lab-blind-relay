 ...
 *    `color-mix()`. This lab's panels and cards are flat custom properties, but
 *    the hero aside is `color-mix(in oklab, var(--accent) 6%, transparent)` over
 *    the page background and the shared top bar mixes both its ink and its
 *    border out of `--accent`. Chromium hands `color-mix()` back to
 *    `getComputedStyle` unresolved, so axe files those nodes under `incomplete`
 *    rather than measuring them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here that is the hero
 *    subtitle `.cl-hero-sub` at `opacity: .85`. The gate this replaces asserted
 *    on axe's `violations` array and on one bespoke 1.4.11 control-border
 *    measurement, and on nothing else — no text-contrast oracle of its own at
 *    all.
 *
 ...
 *    box is `.cmp-wrap`, the `overflow: auto` frame around the four-way
 *    architecture comparison, whose table declares `min-width: 720px` and so is
 *    wider than its frame on any viewport under about 780px. Content scrolled
 *    past it is not dimmed or partly drawn — it is absent from the frame, and
 ...
 *    columns that ARE in view, which are measured for real.
 *
 ...
 *    nothing to do. It is kept because the alternative is a helper that silently
 *    mismeasures the first labelled diagram anyone adds: an ancestor-only walk
 *    would composite a `<text>` onto the `<svg>`'s own transparent background
 *    rather than onto the `<rect>` drawn under it, and report a ratio nothing on
 *    screen has.
 */
 ...
 * uses. One state here IS transient — auto-play advances a step every 1600ms —
 * and the drive handles that by stopping the timer and scanning the settled
 * page rather than by narrowing the walk. The parameter is
 * kept because the narrowed form is what makes a short-lived state measurable
 * at all: a class that lives 500ms is shorter than a full axe pass, so a
 * whole-page scan would catch it only sometimes, and a sometimes-measured
 * assertion is worse than none. Scoped to one subtree the walk costs a couple
 * of milliseconds, so a caller can set the state, measure, and confirm the
 * state is still set.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks four party cards of fact rows, two full key
     * schedules printed as space-grouped hex, two four-row correlation logs, a
     * five-row five-column architecture comparison and the verdict pairs, and
     * every one of those nodes re-walks the same handful of
     * ancestors up to `<body>`. Without the caches the pass re-reads the same
     * computed styles and rects thousands of times, and the gate scans a dozen
     * or more states per configuration. Nothing mutates the DOM during the pass, so the cached
     ...
     * that is `.cmp-wrap`, the frame around the `min-width: 720px` architecture
     * comparison table.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab draws no SVG figure of its own, so the guard
      // is dormant here; it is kept so the first one anyone adds is measured
      // against the shape under it rather than against a stroke's phantom fill.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab's case is its
      // four `<details class="expert">` explainers, all shut on arrival; the
      // gate opens each by clicking its `<summary>` — the route a reader has —
      // rather than setting `.open` from script, which is what the gate this
      // replaces did to all four at once.
      // It also catches `#wire-chip`, which is `hidden` until an exchange runs.
      ...
      // header's `.cl-skip-link` parks at `top:-3rem` and slides in only on
     ...
     * hamburger and GitHub marks, each party card's emoji, the switch `.knob`,
     * and the whole `.wire` strip, which is `aria-hidden` because the position
     * it marks is already stated in each party's `aria-label` ("(message is
     * here)") — would be
     * measured as if each took its element's declared `color`, reporting ratios
     * for marks that carry no text at all. Note the boundary cuts both ways and
     * is a known gap: text that is `aria-hidden` but still painted is skipped by
     * BOTH oracles, so nothing here may hide a real value behind that
     * attribute. `.wire` is the one to watch: it is the only `aria-hidden`
     * subtree here that carries WORDS rather than a glyph. Its chip names the
     * message form on the wire at the current step, and that string is also
     * printed in `#step-status`, the `role="status"` line the same repaint
     * writes — so the strip really is a redundant visual position marker, and
     * it is also `display: none` below 900px where nobody could read it anyway.
     */
     ...
     * cheap to make and silent, and this page builds every one of its sections
     * by interpolating into template literals, which is exactly where it
     * happens: interpolating a string ARRAY into a template literal
     * (`<g>${dots}</g>`) makes JS join it with commas, leaving a run of ","
     * text nodes inside the group. They are invisible on screen, but a walk that
     * measures them reports a 1:1 failure against the panel — a ratio describing
